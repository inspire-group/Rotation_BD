...
class NAD():
    ...
    def __init__(self,angle, name, s_model_path, net, train_loader, test_loader_clean,test_loader_bd , teacher_epochs=1, erase_epochs=20):
    ...
    def train_teacher(self):
        ...
        for epoch in range(0, self.teacher_epochs):
            ...
            self.adjust_learning_rate(optimizer, epoch, 0.001)
    ...
    def train_erase(self):
        ...
        teacher = get_pretrained_vggface(return_activation=True)
        t_model_path = os.path.join(self.folder_path, 'NAD_T_'+self.name+'.pt')
        ...
        student = get_pretrained_vggface(return_activation=True)
    ...
    def test(self, model, criterion, epoch):
        ...
        for idx, (img, target) in enumerate(self.test_loader_bd, start=1):
            ...
            with torch.no_grad():
                poison_output,_,_,_ = model(poison_img)
    ...
    def test_erase(self, nets, criterions, betas, epoch):
        ...
        for idx, (img, target) in enumerate(self.test_loader, start=1):
            ...
            with torch.no_grad():
                output_s, _, _, _ = snet.forward(img)
        ...
        for idx, (img, target) in enumerate(self.test_loader_bd, start=1):
            ...
            with torch.no_grad():
                output_s, activation1_s, activation2_s, activation3_s = snet.forward(poison_img)
                _, activation1_t, activation2_t, activation3_t = tnet.forward(poison_img)
    ...
    def train_step_erase(self, train_loader, nets, optimizer, criterions, betas, epoch):
        ...
        for idx, (img, target) in enumerate(train_loader, start=1):
            ...
            output_s, activation1_s, activation2_s, activation3_s = snet.forward(img)
            _, activation1_t, activation2_t, activation3_t = tnet.forward(img)
