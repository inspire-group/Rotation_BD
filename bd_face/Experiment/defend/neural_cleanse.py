...
# Neural Cleanse!
class NC():
    def __init__(self, args, model, loader, epoch: int = 10, batch_size = 32,
        ...
        # My configuration
        self.folder_path = 'dfresult/NC/'+'newrb'+str(args.trigger_type)+'/'
    ...
    def get_potential_triggers(self):#-> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        file_path = os.path.normpath(os.path.join(
            self.folder_path, 'neural_cleanse_%s.npz' % 0))
        ...
        # for label in [self.target_class]:
            ...
            mark_path = os.path.normpath(os.path.join(
                self.folder_path, 'mark_neural_cleanse_class=%d_%s.png' % (label, 0)))
            mask_path = os.path.normpath(os.path.join(
                self.folder_path, 'mask_neural_cleanse_class=%d_%s.png' % (label, 0)))
            trigger_path = os.path.normpath(os.path.join(
                self.folder_path, 'trigger_neural_cleanse_class=%d_%s.png' % (label, 0)))
