...
class STRIP():
    ...
    def __init__(self, opt, model, loader, strip_alpha: float = 0.5, N: int = 64, defense_fpr: float = 0.1, batch_size=128):

        self.strip_alpha: float = strip_alpha
        self.N: int = N
        self.defense_fpr = defense_fpr
        self.folder_path = 'dfresult/STRIP/'
    ...
    def detect(self,test_loader,test_bad_loader):
        clean_entropy = []
        poison_entropy = []
        ...
        test_bad_loader = tqdm(test_bad_loader)
        for _input, _label in test_bad_loader:
            _input, _label = _input.cuda(), _label.cuda()
            poison_entropy.append(self.check(_input, _label))
        ...
        result_file = self.folder_path+str(self.opt.trigger_type)+str(self.opt.inject_portion) +'strip.npy' 
        ...
        plt.legend()
        hist_file = self.folder_path+str(self.opt.trigger_type)+str(self.opt.inject_portion) +'strip.png' 
        plt.tight_layout()
